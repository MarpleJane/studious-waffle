<template>
    <div>
        ...
        <el-table v-loading="loading" :data="kamiData" style="width: 100%">
...
export default {
    ...
    methods: {
        ...
        handleCurrentChange(val) {
            ...
            this.$axios.post("/v1/chapter/kamisama", {current_page: that.current_page}, that.headers)
              .then(response => {
                  let data = response.data
                  let ret = data.ret
                  if (ret == 1) {
        ...
        fetchData() {
            let that = this
            this.$axios.get("/v1/chapter/kamisama?page_size="+that.page_size)
              .then(response => {
                  let data = response.data
                  console.log(data)
                  let ret = data.ret
