...
export default {
    ...
    methods: {
        ...
        handleEdit(index, row) {
            this.$router.push({name: 'EditDregsExp', params:{form: row}, query:{plot_id: row.id}})
        },
        handleDel(index, row) {
            let that = this
        ...
        handleCurrentChange(val) {
            ...
            this.$axios.post("/v1/simulation/dregs_experience", {current_page: that.current_page}, that.headers)
              .then(response => {
                  let data = response.data
                  let ret = data.ret
                  if (ret == 1) {
                      ...
                      that.dregsExpData = result.items
                  }
              })
              .catch(response => {
                  that.$message("出错啦")
        ...
        fetchData() {
            let that = this
            this.$axios.get("/v1/simulation/dregs_experience?page_size="+that.page_size)
              .then(response => {
                  let data = response.data
                  console.log(data)
                  let ret = data.ret
